...
if __name__=='__main__':
        def tool_chalu():
          try:
            os.system('clear')
            X=(__import__('pyfiglet').figlet_format('ig report',font='slant'))
            print(f'[purple]{X}')
            ...
            print('''[green]
[yellow][1] [green]instagram report 
[yellow][2] [green]instagram report [ [yellow]Lite [green]]
[yellow][3] [green]contact Developer				
            ''')
            choice=input('''\033[32;m$$ Choose --> ''')
            if choice=='1':
              while True:								
                REportt=kreji.Send_Report(Sessionid,crf_tt,USER_E)
                print(REportt)														
            elif choice=='2':
              while True:
                USER_lite=kreji.lite_report(Sessionid,crf_tt,USER_E)
                print(USER_lite)

            elif choice=='3':
              webbrowser.open('https://t.me/PokiePy')
              exit('You have been redirected to developer.')		
            else:
              tool_chalu()
          except Exception as e:
            print(e)		
